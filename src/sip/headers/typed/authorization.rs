    ...
    Ok(match find_param(params, "qop") {
        Some(q) if q.eq_ignore_ascii_case("auth") => Some(AuthQop::Auth {
            cnonce: find_param(params, "cnonce")
                .ok_or_else(|| Error::InvalidParam("missing cnonce".into()))?.into(),
            nc: u8::from_str_radix(
                find_param(params, "nc")
                    .ok_or_else(|| Error::InvalidParam("missing nc".into()))?,
                16,
    ...
    pub fn parse(s: &str) -> Result<Self, Error> {
        ...
        Ok(Authorization {
            ...
            username: find_param(params, "username")
                .ok_or_else(|| Error::InvalidParam("missing username".into()))?.into(),
            ...
            response: find_param(params, "response")
                .ok_or_else(|| Error::InvalidParam("missing response".into()))?.into(),
...
impl std::fmt::Display for Authorization {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{} username=\"{}\", realm=\"{}\", nonce=\"{}\", uri=\"{}\", response=\"{}\"",
        ...
        if let Some(q) = &self.qop { write!(f, ", {}", q)?; }
        Ok(())
    }
}

impl std::convert::From<Authorization> for String {
    fn from(a: Authorization) -> String { a.to_string() }
}

impl std::convert::From<Authorization> for Header {
    fn from(a: Authorization) -> Header {
        Header::Authorization(crate::sip::headers::untyped::Authorization::new(a.to_string()))
    }
}

impl<'a> super::TypedHeader<'a> for Authorization {}
